...
import fr.unantes.sce.trivial.application.StartGame;
import fr.unantes.sce.trivial.enumeration.Cursor;
import fr.unantes.sce.trivial.enumeration.Resolution;
...
public class SettingsGP extends GridPane{
	...
	public CheckBox getCbFullscreen() {
		if(cbFullscreen == null) {
			...
			cbFullscreen.setOnAction(event -> {
				...
				fr.unantes.sce.trivial.vue.MenuGP.getTitle().setFitWidth(stage.getWidth()/1.5);
				fr.unantes.sce.trivial.vue.MenuGP.getTitle().setFitHeight(stage.getHeight()/2.);
			});}
		return cbFullscreen;
	}
	//Slider
	...
	public void setSizeScreen() {
		if(!getCbFullscreen().isSelected()) {
	        ...
	        //Changer la taille du titre
			fr.unantes.sce.trivial.vue.MenuGP.getTitle().setFitWidth(Integer.parseInt(width));
			fr.unantes.sce.trivial.vue.MenuGP.getTitle().setFitHeight(Integer.parseInt(height)/3.);
