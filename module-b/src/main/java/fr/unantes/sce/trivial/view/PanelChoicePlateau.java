...
import fr.unantes.sce.trivial.model.Play;
import fr.unantes.sce.trivial.model.Question;
import fr.unantes.sce.trivial.model.SquareQuestionCheese;
...
public class PanelChoicePlateau extends JPanel
{
	...
	public PanelChoicePlateau()
	{
		...
		//cr�ation des boutons des choix de d�placement de 2 � 7 boutons
		int j=1;
...
class PlateauChoice extends JPanel
{
	...
	public void paintComponent(Graphics g){
		//nettoie le contenu de la fen�tre pour �viter des traces des pr�c�dents paint
		//g.clearRect(0, 0, getWidth(), getHeight());
		
		//correspond aux coordonn�es x de toutes les cases du plateau
		int x[][]= CoordonneePlateau.getPlateauX();
		
		
		//correspond aux coordonn�es y de toutes les cases du plateau
		int y[][]= CoordonneePlateau.getPlateauY();
		
		//correpond aux coordonn�es x des formes dans la case centrale
		int caseDepx[][]= CoordonneePlateau.getCaseDepX();
		
		//correpond aux coordonn�es y des formes dans la case centrale
		int caseDepy[][]= CoordonneePlateau.getCaseDepY();
		
		//correspond aux coordonn�es x des curseurs
		int curseurX[][]= CoordonneePlateau.getCurseurX();
		
		//correspond aux coordonn�es y des curseurs
		int curseurY[][]= CoordonneePlateau.getCurseurY();
		
		//variable compteur utilis� pour la s�lection des cat�gories des cases camemberts
		int j=1;
		
		//dessine la case centrale		
		//dessine les formes � l'int�rieur de la case centrale
		for(int i=0;i<6;i++){
		...
		for(int i=1;i<=72;i++){
			...
			if(i==6 || i==18 || i==30 || i==42 || i==54 || i==66){
				...
				//couleur blanche pour les cases rejou�s
				...
				// id repr�sente l'id de la case o� se situe le joueur
