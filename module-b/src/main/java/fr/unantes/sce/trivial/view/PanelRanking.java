		...
		if(jcTypeRanking==null){
			...
			jcTypeRanking.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent arg0) {
					...
					if(selection.equals("Ranking point")){// Si c'est la valeur "Ranking point de la JComboBox qui est s�lectionn�e
						panelC.getPanelCenter().getCard().show(panelC.getPanelCenter(), "jtableRankingPoint"); 
						// On montre la JCombobox cr��e avec la class ModelTableAdapterRankingPoint
					}
					else if(selection.equals("Ranking victory")){//Si c'est la valeur "Ranking victory de la JComboBox qui est s�lectionn�e
...
class PanelCenter extends JPanel 
{
	...
	public PanelCenter()
	{
		...
		// Le PanelCenter contient un cardLayout, ce qui permet de passer d'une JTable � une autre.
	...
	public JTable getJtableRankingPoint() 
	{
		if(jtableRankingPoint==null)
		{
			...
			// On cr�� la JTable sur base de la classe "ModelTableAdapterRankingPoint"
	...
	public JTable getJtableRankingVic() 
	{
		if(jtableRankingVic == null)
		{
			...
			jtableRankingVic.setAutoCreateRowSorter(true);// Voir la m�thode d'au dessus.
		}
