...
import fr.unantes.sce.trivial.exception.FieldEmptyException;
import fr.unantes.sce.trivial.exception.PlayerExistsAlreadyException;
...
class PanelButton extends JPanel implements ActionListener
{
	...
	public void actionPerformed(ActionEvent e)
	{
		// Permet de verifier toutes les entr�es du formulaire
		...
		} catch (BirthdayGamerException | EmailGamerException | FieldEmptyException | PlayerExistsAlreadyException e1) 
		{
			...
			JOptionPane.showMessageDialog(null, e1.getMessage(), "Pas de cr�ation", JOptionPane.ERROR_MESSAGE);
		}
...
class PanelJoueurEntry extends JPanel implements ActionListener
{
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		if(jrExistsPlayer.isEnabled());
		{
			...
			if (option == 1 || option == 2) // si la connection est valid�e alors on ouvre la page de jeu
			{
...
class PanelNewPlayer extends JPanel
{
	...
	public Gamer verifyFormAndGetIt() throws BirthdayGamerException, EmailGamerException, FieldEmptyException, PlayerExistsAlreadyException
	{
		...
		{	
			...
			//on cree le nouveau joueur, on voit si une exception se propage ou si le joueur est cr��
		...
		{
			//cas o� un des champs est vide
			throw new FieldEmptyException();
			
		}
		
		
		//Ensuite si on est ici, le joueur est cr�� il faut ensuite verifier qu'il n'existe pas d�j� pour le login
