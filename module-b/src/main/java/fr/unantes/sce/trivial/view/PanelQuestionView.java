...
import fr.unantes.sce.trivial.utility.Audio;
...
import fr.unantes.sce.trivial.model.SquareBegin;
...
public class PanelQuestionView extends JPanel implements ActionListener
{
	...
	private String selection; //String du bouton selectionn�
	...
	public PanelQuestionView()
	{
		...
		//Panel compos� de la question, des 4choix de reponses sous forme de boutons
		
		
		JPanel gl = new JPanel();
		
		//Panel des 4boutons - r�ponse
		gl.add(getJpChoices());
...
//		
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		if(selection==answer){
			...
			//r�ponse correcte sur la case centrale
			...
			//r�ponse correcte � une simple case question
			else
			{
			...
			//test pour savoir si la partie est termin�
			if(Play.isGameFinish(Game.getInstance()))
			{
				...
				//reset des donn�es de l'instance Game et de l'instance Piece pour permettre de lancer une nouvelle partie sans �tre oblig� de fermer l'application
//				Game.resetInstance();
//				Piece.resetInstance();
				//retour � la fen�tre d'accueil
