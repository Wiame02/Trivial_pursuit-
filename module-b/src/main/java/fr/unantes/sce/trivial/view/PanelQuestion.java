...
import fr.unantes.sce.trivial.model.Observer;
...
class PanelButtons extends JPanel
{
	...
	public JButton getjbSupprimer()
	{
		if(jbSupprimer == null)
		{
			...
			jbSupprimer.setVisible(false); //invisible au debut car il faut etre connect� en admin pour y acceder
			jbSupprimer.addActionListener(new ActionListener() 
			{
				...
				public void actionPerformed(ActionEvent e)
				{
					...
					for(int i=rows.length-1;i>=0;i--) // boucle en sens inverse pour �viter le bug du row lors ce que l'on selectionne plusieurs items
					{							
	...
	public JButton getJbAjouter()
	{
		if(jbAjouter == null)
		{
			...
			jbAjouter.setVisible(false); //invisible au debut car il faut etre connect� en admin pour y acceder
