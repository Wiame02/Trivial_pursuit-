	...
	public int isVerifyLoginAdmin()
	{
		...
		while((!isValide) && option == 0) // boucle tant qu'on ne clique pas sur cancel ou tant que le mdp et login sont incorrects
		{
			...
			if(option == 0) // Si option = OK
			{
				...
				//on test les utilisateurs avec le mot de passe et le login envoy�
				...
				{	
					JOptionPane.showMessageDialog(null, "Wrong login or password, please try again", "Pas de cr�ation", JOptionPane.ERROR_MESSAGE);
